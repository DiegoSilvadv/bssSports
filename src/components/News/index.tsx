...
export default function News({ data }) {
  ...
  const [dataPosts, setDataPosts] = useState([
    {
      title: "Java basquete é campeão da Taça Curitiba",
      hour: "3 horas atras",
      image: "1213"
    },
    {
...
function ListItem({ posts }: IPosts) {
  return (
    <SafeAreaView>
      ...
      <S.BlogFooterConatainer>
      <S.BlogWrapperFooter>
        <S.BlogWrapperFooterTitle>{posts.title}</S.BlogWrapperFooterTitle>
        <S.BlogWrapperFooterInfos>
          <S.ImageFooterPost source={miniLogo} />
          <S.PostFooterTitle>BSS Sports há {posts.hour}</S.PostFooterTitle>
        </S.BlogWrapperFooterInfos>
      </S.BlogWrapperFooter>
      <S.ImagePost source={BlogImage} />
      </S.BlogFooterConatainer>
    </SafeAreaView>
